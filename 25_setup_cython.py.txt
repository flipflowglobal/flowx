...
Rust lib must be compiled first (or set build=True to auto-compile):
    cargo build --manifest-path hinsdale/Cargo.toml --release --lib
    → hinsdale/target/release/libhinsdale.so
"""
...
def ensure_rust():
    ...
    r = subprocess.run(
        ["cargo", "build", "--manifest-path", str(RUST_DIR / "Cargo.toml"), "--release", "--lib"],
        cwd=RUST_DIR,
        env=env,
    )
    if r.returncode != 0:
